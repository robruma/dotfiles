...
git fetch
if ! (git remote update && git diff --exit-code --quiet origin/${BRANCH}) || [[ $(git submodule update --recursive --remote > /dev/null 2>&1; git submodule status --recursive) =~ ^+.* ]] || [[ -n $(git status --porcelain) ]]; then
